...
public class OligoX
{
     ...
     *@return OligoX object
     */
    public OligoX(String oligo)
    {
        this.oligo = oligo;
    ...
    public OligoX(StringBuilder oligo)
    {
    ...
    public OligoX(StringBuffer oligo)
    {
        this.oligo = oligo.toString();
        OLIGO_LENGTH = oligo.length();
    }
...
//=========================| OVER RIDDEN METHODS |============================//
     ...
     *@return OligoX object - oligo in lowercase
     */
    public OligoX toLowerCase()
    {
        return new OligoX(oligo.toLowerCase());
    }
...
//=================================| METHODS |================================//
     ...
     */
    public OligoX antiparallel()
    {
        OligoX tempOligo = new OligoX(oligo);
        OligoX complement = new OligoX(tempOligo.complement());
     ...
     *@return OligoX object representing best match; otherwise, throws MatchNotFoundException
     *@throws OligoException
     */
    public OligoX binarySearch(OligoX query) throws OligoException
    {
        ...
        OligoX searchWindow = new OligoX(oligo);
        OligoX searchKey = query;
        OligoX[] splitKey = new OligoX[2];
        OligoX leftKey = new OligoX();
        OligoX rightKey = new OligoX();
        OligoX remainingKey = new OligoX();
...
//                        System.out.println();
                ...
                else if(foundLeftKey && foundRightKey)
                {
                    ...
                    if(!remainingKey.isEmpty())
                    {
                        ...
                        searchWindow = new OligoX(oligo);  //new searchWindow is entire the initial entire source sequence
...
//                    System.out.println();
     ...
     */
    private static OligoX assembleKey(ArrayList inputKey)
    {
        ...
        return new OligoX(leftKey.append(middleKey.append(rightKey)));
    } //end assembleKey()
     ...
     *@@return OligoX object
     */
    private static OligoX[] halve(OligoX query)
    {
        OligoX target = query;
        OligoX[] output = new OligoX[2];
        output[0] = new OligoX(target.toString().substring(0, target.toString().length()/2));    //obtain left half of string
        output[1] = new OligoX(target.toString().substring(target.toString().length()/2));   //obtain right half of string
        return output;
    }
     ...
     */
    private static boolean willNeverFind(OligoX target, OligoX source)
    {
     ...
     *
     */
    public OligoX complement()
    {
        String tempOligo = "";
        char[] source = oligo.toLowerCase().toCharArray();
    ...
    public int countChar(char inputChar)
    {
        ...
        for(int i = 0; i <= OLIGO_LENGTH - 1; i++)
        {
            if(oligo.toLowerCase().charAt(i) == ch || oligo.charAt(i) == ch)
                count++;
        }
        return count;
    }
     ...
     *@return OligoX object - resulting sequence after excision; otherwise throws MatchNotFoundException
     *@throws OligoException
     */
    public OligoX exciseLeftFrom(OligoX query, int mismatches) throws OligoException
    {
        if(isFuzzyMatch(query, mismatches))
        {
            int index = getFirstMatchCoordinate(query, mismatches);
            return new OligoX(oligo.substring(index + query.length()));
        }
    ...
    public OligoX exciseLeftFrom(OligoX query, int mismatches, int ins, int del, int minKeyLength) throws OligoException
    {
        if(isFuzzySmithWatermanMatch(query, mismatches, ins, del, minKeyLength))
        {
            OligoX swKey = getFuzzySWkey(query, mismatches, ins, del, minKeyLength);
            return new OligoX(exciseLeftFrom(swKey, mismatches));
        }
        else throw new OligoException(mnf, "exciseLeftFrom()"); // exhausted all possibilities, no matches found
    } // end exciseLeftFrom()
    ...
    /**
     *Excises everthing left of query, excluding the query sequence
     *@param query search key
     *@param mismatches max number of allowed mismatches
     *@return OligoX object - resulting sequence after excision; otherwise throws MatchNotFoundException
     *@throws OligoException
     */
    public OligoX exciseLeftOf(OligoX query, int mismatches) throws OligoException
    {
        if(isFuzzyMatch(query, mismatches))
        {
            int index = getFirstMatchCoordinate(query, mismatches);
            return new OligoX(oligo.substring(index));
        }
    ...
    public OligoX exciseLeftOf(OligoX query, int mismatches, int ins, int del, int minKeyLength) throws OligoException
    {
     ...
     *@throws OligoException
     */
    public OligoX exciseRightFrom(OligoX query, int mismatches) throws OligoException
    {
        if(isFuzzyMatch(query, mismatches))
        {
            int index = getFirstMatchCoordinate(query, mismatches);
            return new OligoX(oligo.substring(0, index));
        }
    ...
    public OligoX exciseRightFrom(OligoX query, int mismatches, int ins, int del, int minKeyLength) throws OligoException
    {
        int index = getFirstMatchCoordinate(query, mismatches, ins, del, minKeyLength);
        if(index != -1)
            return new OligoX(oligo.substring(0, index));
    ...
    /**
     *Excises everthing right of query, excluding the query sequence
     *@param query search key
     *@param mismatches max number of allowed mismatches
     *@return OligoX object - resulting sequence after excision; otherwise throws MatchNotFoundException
     *@throws OligoException
     */
    public OligoX exciseRightOf(OligoX query, int mismatches) throws OligoException
    {
        if(isFuzzyMatch(query, mismatches))
        {
            int index = getFirstMatchCoordinate(query, mismatches);
            return new OligoX(oligo.substring(0, index + query.length()));
        }
    ...
    public OligoX exciseRightOf(OligoX query, int mismatches, int ins, int del, int minKeyLength) throws OligoException
    {
        if(isFuzzySmithWatermanMatch(query, mismatches, ins, del, minKeyLength))
        {
            int index = getFirstMatchCoordinate(query, mismatches, ins, del, minKeyLength);
            OligoX swKey = getFuzzySWkey(query, mismatches, ins, del, minKeyLength);
            return new OligoX(oligo.substring(0, index + swKey.length()));
        }
     ...
     *@return OligoX object - subsequence of oligo, if found; otherwise throws MatchNotFoundException
     *@throws MatchNotFoundException
     */
    public OligoX extractSequence(OligoX query, int mismatches) throws OligoException
    {
        if(isFuzzyMatch(query, mismatches))
        {
            if(getFirstMatchCoordinate(query, mismatches) != -1)
            {
                ...
                return new OligoX(oligo.substring(start, stop));
            }
    ...
    public OligoX extractSequence(OligoX query, int mismatches, int ins, int del, int minKeyLength) throws OligoException
    {
        OligoX swKey = new OligoX();
        try
        {
     ...
     *@param end end index
     *@return OligoX object  representing extracted sequence, if found; otherwise throws OligoException
     *@throws OligoException
     */
    public OligoX extractSequence(int start, int end) throws OligoException
    {
        String s = new String();
     ...
     *@return OligoX object
     */
    public static OligoX generateRandomOligo(int length)
    {
     ...
     */
    public ArrayList getAllMatchCoordinates(OligoX query, int mismatches)
    {
        OligoX searchWindow = new OligoX();
        ArrayList hits = new ArrayList();
        final int QUERY_LENGTH = query.length();
        
        for(int i = 0; i + QUERY_LENGTH <= OLIGO_LENGTH; i++)
        {
            searchWindow = new OligoX(oligo.substring(i, i + QUERY_LENGTH));
     ...
     */
    public ArrayList getAllMatchCoordinates(OligoX query, int mismatches, int ins, int del, int minKeyLength)
    {
        OligoX searchWindow = new OligoX();
        OligoX swKey = new OligoX();
        ArrayList hits = new ArrayList();
        ...
        for(int i = 0; i + QUERY_LENGTH <= OLIGO_LENGTH; i++)
        {
            try
            {
                ...
                searchWindow = new OligoX(oligo.substring(i, i + swKey.length())); //narrow searchWindow to length of potential key
            }
     ...
     */
    public int getFirstMatchCoordinate(OligoX query, int mismatches)
    {
     ...
     *@return int index if found; otherwise returns -1
     */
    public int getFirstMatchCoordinate(OligoX query, int mismatches, int ins, int del, int minKeyLength)
    {
     ...
     *isFuzzySmithWatermanMatch(OligoX query, int mismatches, int ins, int del, int minKeyLength) and simply returns the key equivalent.
     *@param query query sequence
     *@param mismatches maximum number of allowedMismatches
     *@param ins maximum allowed number of inserts in SOURCE sequence
     *@param del maximum allowed number of deletes in SOURCE sequence
     *@param minKeyLength minimum length of found key in order for search to be considered successful
     *@return [OligoX object] Found key
     *@throws OligoException
     *@see also isFuzzySmithWatermanMatch(OligoX query, int mismatches, int ins, int del, int minKeyLength)
     */
    public OligoX getFuzzySWkey(OligoX query, int mismatches, int ins, int del, int minKeyLength) throws OligoException
    {
        if(isFuzzySmithWatermanMatch(query, mismatches, ins, del, minKeyLength))
     ...
     */
    public int getLastMatchCoordinate(OligoX query, int mismatches)
    {
        ArrayList list = getAllMatchCoordinates(query, mismatches);
     ...
     *@return int representing the found index; otherwise returns -1
     */
    public int getLastMatchCoordinate(OligoX query, int mismatches, int ins, int del, int minKeyLength)
    {
        ArrayList list = getAllMatchCoordinates(query, mismatches, ins, del, minKeyLength);
        return (list.size() > 0) ? Integer.parseInt(list.get(list.size() - 1).toString()) : -1;
    }
     ...
     *@return OligoX object with inserted sequence
     */
    public OligoX insert(OligoX insert, int index)
    {
        String insertSeq = insert.toString();
        StringBuffer targetOligo = new StringBuffer(oligo);
        return new OligoX(targetOligo.insert(index, insertSeq));
    }
    ...
    {
        char[] s = oligo.toLowerCase().toCharArray();
        for(char base: s)
        {
     ...
     *@return boolean
     */
    public boolean isFuzzyMatch(OligoX inputQuery, int mismatches)
    {
        char[] query = inputQuery.toLowerCase().toCharArray();
        char[] source = oligo.toLowerCase().toCharArray();
     ...
     *@see smithWaterman()
     */
    public boolean isFuzzySmithWatermanMatch(OligoX query, int mismatches, int ins, int del)
    {
        OligoX swKey = new OligoX();
    ...
    public boolean isFuzzySmithWatermanMatch(OligoX query, int mismatches, int ins, int del, int minKeyLength)
    {
        OligoX swKey = new OligoX();
     ...
     *@return int index (if found); otherwise, returns -1
     */
    public int lastIndex(OligoX query, int mismatches)
    {
    ...
    public OligoX ligate(OligoX input)
    {
        return new OligoX(oligo + input.toString());
    }
     ...
     *@return OligoX object - mutated oligo sequence
     */
    public OligoX mutate(int percent)
    {
     ...
     *@return [OligoX object] randomized oligo sequence
     */
    public OligoX randomizeOligo()
    {
        StringBuffer tempOligo = new StringBuffer(oligo);
     ...
     *@return OligoX object: oligo of zero length
     */
    public OligoX resetToNull()
    {
        return new OligoX();
    }
     ...
     *@return OligoX object: the reversed oligo
     */
    public OligoX reverse()
    {
        StringBuilder tempOligo = new StringBuilder(oligo);
        return new OligoX(tempOligo.reverse());
    }
     ...
     *@return OligoX object: transformed query sequence, if within specified conditions. Otherwise, returns "smithWaterman(): NotFoundException"
     *@throws OligoException
     */
    public OligoX smithWaterman(OligoX query, int ins, int del) throws OligoException
    {
        char[] s = ("x" + oligo).toLowerCase().toCharArray(); //pad 'x' as first char of oligo
        char[] q = ("x" + query).toLowerCase().toCharArray(); //pad 'x' as first char of query
     ...
     *@return OligoX object: spliced oligo, if target is found
     */
    public OligoX spliceOut(OligoX query, int mismatches) throws OligoException
    {
        if(isFuzzyMatch(query, mismatches))
        {
            int index = getFirstMatchCoordinate(query, mismatches);
            return new OligoX(oligo.substring(0, index) + oligo.substring(index + query.length(), oligo.length() - 1));
        }
     ...
     *@param start start index
     *@param end end index
     *@return OligoX object representing remaining sequence after splicing
     *@throws OligoException
     */
    public OligoX spliceOut(int start, int end) throws OligoException
    {
        String s = "";
        boolean startIndexOutofBounds = start < 0;
        boolean endIndexOutofBounds = end > oligo.length() - 1;
